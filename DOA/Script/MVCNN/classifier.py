...
def double_conv1(in_c, out_c):
    conv = nn.Sequential(
        ...
        nn.Dropout(p=0.3),
        nn.Conv2d(out_c, out_c, kernel_size=(4,2),stride=1, bias=True),
        nn.BatchNorm2d(out_c),
        nn.ReLU(inplace=True))
    return conv
...
class Recurrent_block(nn.Module):
    def __init__(self,ch_out,t=2):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(ch_out, ch_out, kernel_size=(3, 3), stride=1, padding=1, bias=True),
...
class cnn2(nn.Module):
    ...
    def forward(self, image):
        ...
        for i in range(0,3):
            ...
            # print(cnn1_out.size())
            trained_channels.append(cnn1_out)
        stack_ch = torch.stack(trained_channels)
        print(stack_ch.size())
        pooled_views, _ = torch.max(stack_ch, dim=0)
        print(pooled_views.size())

        x1 = self.RCNN1(pooled_views)
