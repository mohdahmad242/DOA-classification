...
class Reduction_A(nn.Module):
    ...
    def __init__(self, in_channels, k, l, m, n):
        ...
        self.branch_0 = Conv2d(in_channels, n, 4, stride=2, padding=1, bias=False)
        self.branch_1 = nn.Sequential(
            Conv2d(in_channels, k, 4, stride=1, padding=1, bias=False),
            Conv2d(k, l, 3, stride=1, padding=1, bias=False),
            Conv2d(l, m, 3, stride=2, padding=1, bias=False),
        )
        self.branch_2 = nn.MaxPool2d(2, stride=2, padding=0)
...
class Stem(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv2d_1a_3x3 = Conv2d(in_channels, 32, 1, stride=2, padding=0, bias=False)

        self.conv2d_2a_3x3 = Conv2d(32, 32, 1, stride=1, padding=0, bias=False)
        self.conv2d_2b_3x3 = Conv2d(32, 64, 1, stride=1, padding=1, bias=False)

        self.mixed_3a_branch_0 = nn.MaxPool2d(1, stride=2, padding=0)
        self.mixed_3a_branch_1 = Conv2d(64, 96, 1, stride=2, padding=0, bias=False)

        self.mixed_4a_branch_0 = nn.Sequential(
            Conv2d(160, 64, 1, stride=1, padding=0, bias=False),
            Conv2d(64, 96, 1, stride=1, padding=0, bias=False),
        )
        self.mixed_4a_branch_1 = nn.Sequential(
            Conv2d(160, 64, 1, stride=1, padding=0, bias=False),
            Conv2d(64, 64, (1, 1), stride=1, padding=(0), bias=False),
            Conv2d(64, 64, (1, 1), stride=1, padding=(0), bias=False),
            Conv2d(64, 96, 1, stride=1, padding=0, bias=False)
        )
...
class Inception_A(nn.Module):
    def __init__(self, in_channels):
        ...
        self.branch_0 = Conv2d(in_channels, 96, 1, stride=1, padding=1, bias=False)
        self.branch_1 = nn.Sequential(
            Conv2d(in_channels, 64, 2, stride=1, padding=1, bias=False),
            Conv2d(64, 96, 2, stride=1, padding=1, bias=False),
        )
        ...
        self.brance_3 = nn.Sequential(
            ...
            Conv2d(384, 96, 1, stride=1, padding=1, bias=False)
        )
...
class Inceptionv4(nn.Module):
    def __init__(self, in_channels=3, k=192, l=224, m=256, n=384):
        ...
        self.linear = nn.Linear(1536, 181)

    def forward(self, input):
        # print("Input images => ",x.size())
        x = self.features(input)
        # print("features => ",x.size())
        x = self.global_average_pooling(x)
        # print("global_average_pooling",x.size())
        x = x.view(x.size(0), -1)
        # print("View =>", x.size())
        x = self.linear(x)
        x = x.reshape(input.size()[0], 1, 1, 181)
